...
def predict_json(
    ...
    try:
        ...
        preview_text = (text or "")[:50]
        print(f"🔵 [predict_json] Called with text={preview_text!r}, video={video!r}, n_timesteps={n_timesteps}")
...
def run_prediction(input_type, video_file, audio_file, text_input,
    ...
    has_video = input_type == "Video" and video_file is not None
    has_audio = input_type == "Audio" and audio_file is not None
    has_text = input_type == "Text" and (text_input or "").strip()
    if input_type == "Text" and has_text:
        text_input = validate_text_input(text_input)

    if not (has_video or has_audio or has_text):
