...
def predict_json(
    ...
    try:
        ...
        if video is not None:
            ...
            import shutil
            if not any(video_path.lower().endswith(ext) for ext in ALLOWED_VIDEO_EXTENSIONS):
                orig_name = get_original_filename(video)
                if orig_name and any(orig_name.lower().endswith(ext) for ext in ALLOWED_VIDEO_EXTENSIONS):
