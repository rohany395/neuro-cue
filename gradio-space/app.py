...
def predict_json(
    ...
    import traceback
    try:
        print(f"🔵 [predict_json] Called with text={(text or '')[:50]!r}, video={video!r}, n_timesteps={n_timesteps}")
        ...
        if video is not None:
            ...
            # TRIBE validates by extension. Gradio uploads may be extensionless,
            # so copy the upload to an equivalent path with a safe video suffix.
            video_path = ensure_video_extension(video_path, orig_name)
...
def run_prediction(input_type, video_file, audio_file, text_input,
    ...
    if input_type == "Video" and video_file is not None:
        video_file, orig_name = resolve_uploaded_video_path(video_file)
        video_file = ensure_video_extension(video_file, orig_name)
