...
flowchart TD
  ...
  subgraph Vercel["Vercel frontend"]
    ...
    Health["Serverless health endpoint<br/>/api/predict"]
  end
...
- Custom React frontend using the public Hugging Face Space API
...
`npm run dev` starts Vite only. Predictions and uploads use the public Hugging Face Space directly through `@gradio/client`; `/api/predict` is a health endpoint only and rejects inference POSTs.

The deployed Vercel frontend reads the public Space URL at build time:

```bash
VITE_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

Do not expose Hugging Face tokens or shared API secrets with a `VITE_` prefix; Vite bakes those values into the public JavaScript bundle.

**Video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB. The React app uploads video and runs predictions **directly on the Hugging Face Space** (`@gradio/client` + `VITE_SPACE_URL`).
