...
flowchart TD
  ...
  subgraph Vercel["Vercel frontend"]
    ...
    Status["Serverless status route<br/>/api/predict"]
  end
...
- Custom React frontend that calls the public Hugging Face Space directly
...
cd frontend
npm install
cp .env.local.example .env.local   # optional: override VITE_SPACE_URL
npm run dev
```

`npm run dev` starts Vite only. Files under `frontend/api/` are **Vercel serverless functions** and do not run inside Vite. The React app uses `@gradio/client` in the browser to call the public Hugging Face Space (`VITE_SPACE_URL`) for uploads and predictions.

The deployed Vercel API route at `/api/predict` is a status endpoint only. It does not forward prediction requests or use an `HF_TOKEN`.

```bash
VITE_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

**Video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB. The React app uploads video **directly to the Hugging Face Space** (`@gradio/client` + `VITE_SPACE_URL`) and then calls `/predict_json` on the same Space with the uploaded file reference. Text predictions use the same direct Gradio API path.
