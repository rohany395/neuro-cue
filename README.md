...
Only public `VITE_*` variables are used by the frontend:

```bash
VITE_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

Do not put Hugging Face tokens or shared API secrets in `VITE_*` variables; Vite
embeds them into the public JavaScript bundle.
...
(`@gradio/client` + `VITE_SPACE_URL`) and then calls `/predict_json` on the Space.
