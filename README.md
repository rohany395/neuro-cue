...
flowchart TD
  ...
  subgraph Vercel["Vercel frontend"]
    ...
    Client["@gradio/client<br/>browser-side"]
    React --> Client
  end
...
- @gradio/client (public browser access to the Space API)
- Deployed on Vercel
...
- Custom React frontend that calls the public Hugging Face Space API
...
cd frontend
npm install
cp .env.local.example .env.local   # optional: customize VITE_SPACE_URL
npm run dev
```

`npm run dev` starts Vite. The React app connects directly to the public Gradio API with `@gradio/client`; it does not need a server-side prediction proxy or Hugging Face token.

```bash
VITE_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

Only `VITE_*` values are bundled into browser JavaScript. Do not put `HF_TOKEN`, `PREDICT_API_SECRET`, or any other server secret in frontend environment variables. The legacy Vercel `/api/predict` route is disabled for POST requests.

**Video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB. The React app uploads video **directly to the Hugging Face Space** (`@gradio/client` + `VITE_SPACE_URL`) and then calls `/predict_json` on the same Space with the returned upload reference.
