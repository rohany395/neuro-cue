...
The frontend uses `@gradio/client` in the browser to call the public Hugging Face Space. It does not need a Vercel prediction proxy or a browser-visible API key.
...
**Video uploads:** Browser uploads go directly to the Hugging Face Space (`@gradio/client` + `VITE_SPACE_URL`), bypassing Vercel request body limits. Text and video predictions both call the Space's `/predict_json` API directly.
