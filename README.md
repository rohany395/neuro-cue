...
- @gradio/client (browser-direct public Space access)
- Deployed on Vercel
...
- Custom React frontend with direct Hugging Face Space predictions
...
cd frontend
npm install
cp .env.local.example .env.local   # optional: adjust VITE_SPACE_URL
npm run dev
```

`npm run dev` starts Vite only. Files under `frontend/api/` are **Vercel serverless functions** and do not run inside Vite. The React app calls the public Hugging Face Space directly for predictions and video uploads.

The deployed Vercel status endpoint reads optional environment variables:

```bash
HF_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

**Predictions and video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB, and browser-exposed Vite variables are not secrets. The React app uses `@gradio/client` + `VITE_SPACE_URL` to call the public Hugging Face Space directly.
