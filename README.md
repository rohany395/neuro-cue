...
- @gradio/client (direct public Space access)
- Deployed on Vercel
...
- Custom React frontend backed by the public Hugging Face Space API
...
`npm run dev` starts Vite only. The React app calls the public Hugging Face Space directly with `@gradio/client`; no browser-bundled API secret or server-side Hugging Face token is required for predictions. Files under `frontend/api/` are Vercel compatibility endpoints and do not relay prediction POSTs.

Relevant environment variables:

```bash
# Optional server-side value for the disabled /api/predict health endpoint
HF_SPACE_URL=https://rohany395-neuro-cue.hf.space/

# Public browser value used by @gradio/client
VITE_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

**Video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB. The React app uploads video directly to the Hugging Face Space (`@gradio/client` + `VITE_SPACE_URL`) and then calls the Space's `/predict_json` API. Text predictions use the same direct Space API.
