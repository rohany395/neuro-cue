...
- @gradio/client (public Space API access)
- Deployed on Vercel
...
- Custom React frontend with direct Hugging Face Space integration
...
cd frontend
npm install
cp .env.local.example .env.local   # optional: set a custom VITE_SPACE_URL
npm run dev
```

The React app calls the public Hugging Face Space directly with `@gradio/client`.
Do not put Hugging Face tokens or prediction API secrets in `VITE_*`
variables; Vite bakes those values into the public browser bundle.

**Video uploads:** The React app uploads video **directly to the Hugging Face
Space** (`@gradio/client` + `VITE_SPACE_URL`) and then passes the returned
Gradio file reference to `/predict_json`.
