...
cd frontend
npm install
cp .env.local.example .env.local   # optional: edit VITE_SPACE_URL
npm run dev
```

The React frontend calls the public Hugging Face Space with `@gradio/client`.
Set `VITE_SPACE_URL` only if you want to point the UI at a different Space:

```bash
VITE_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

Do not expose `HF_TOKEN` or other server secrets with a `VITE_` prefix; Vite
embeds those values in the browser bundle.

**Video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB. The React app uploads video **directly to the Hugging Face Space** (`@gradio/client` + `VITE_SPACE_URL`) and then runs `/predict_json` on the same public Space.
