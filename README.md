...
- @gradio/client (browser-to-Space API access)
- Deployed on Vercel
...
The React app calls the public Hugging Face Space with `@gradio/client`.
No Hugging Face token or browser-exposed `VITE_*` API key is required for local
development or production Vercel builds. If you need to point the frontend at a
different Space, set:
...
**Video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB. The React app uploads video **directly to the Hugging Face Space** (`@gradio/client` + `VITE_SPACE_URL`), then sends the returned upload reference to the Space `/predict_json` API. Text predictions also call the Space API directly.
