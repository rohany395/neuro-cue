...
The browser calls the public Hugging Face Space with `@gradio/client`; do not put
private Hugging Face tokens or shared API secrets in `VITE_*` variables because
Vite bakes them into the public JavaScript bundle.
...
The React app uploads video and requests predictions **directly from the Hugging
Face Space** (`@gradio/client` + `VITE_SPACE_URL`).
