...
- @gradio/client (direct browser access to the public Space)
- Deployed on Vercel
...
Use `VITE_SPACE_URL` to point local development at a different Space:

```bash
VITE_SPACE_URL=https://rohany395-neuro-cue.hf.space/
```

Do not put private Hugging Face tokens in `VITE_*` environment variables. The
legacy Vercel `/api/predict` proxy is disabled; predictions and video uploads
both go directly to the Space, which validates uploaded video paths server-side.
