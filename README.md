...
- @gradio/client (public Space access from the browser)
- Deployed on Vercel
...
cd frontend
npm install
cp .env.local.example .env.local   # optional; defaults to the public Space
npm run dev
```

`npm run dev` starts Vite. The React app uses `@gradio/client` to upload videos and call `/predict_json` on the public Hugging Face Space directly, so no browser-bundled API key or server-side Hugging Face token is required for frontend development.
...
**Video uploads:** Vercel serverless functions reject request bodies over ~4.5 MB. The React app uploads video **directly to the Hugging Face Space** (`@gradio/client` + `VITE_SPACE_URL`) and then predicts against the returned Gradio upload reference.
